...
static int handle_connection(struct socket_app_state *s)
{
  /*
   if(uip_newdata()) 
   {
      ...
      while(!uip_closed())
      {
      PSOCK_READTO(&s->p, 0xfe);
      memcpy(buffer,s->inputbuffer,20);  
      //buffer[0] = s->inputbuffer[0];
      //buffer[1] = s->inputbuffer[1];
      //buffer[2] = s->inputbuffer[2];       
  ...
  */
    ...
    while (1) 
    {
     PSOCK_READTO(&s->p, 0xfe);
      ...
      memcpy(buffer,s->inputbuffer,20);    
      memset(s->inputbuffer, 0x00, sizeof(s->inputbuffer)); // Empty buffer
    }    
    PSOCK_CLOSE(&s->p); // Close the connection
    PSOCK_END(&s->p);
